...
import {
  ...
  formatFromMatrixToIRC,
  formatFromMatrixToDiscord
} from './formatting';
...
const MATRIX_BOT_REPLY_PATTERN = /^> <@.+:.+> <b>\[(.+@.+)\]<\/b> (.+)\n\n(.+)/s;
const MATRIX_REPLY_PATTERN = /^> <(@.+):.+> (.+)\n\n(.+)/s;
...
class Bot {
  constructor(options) {
    ...
    this.formatIRCText = this.format.ircText || '<{$displayUsername}@Discord> {$text}';
    ...
    this.formatDiscord = this.format.discord || '**<{$author}@IRC>** {$withMentions}';
  ...
  attachListeners() {
    ...
    this.discord.on('message', (message) => {
      ...
      if (message.reference !== null) {
        const originalMsg = message.reference.messageID;
        message.channel.messages.fetch(originalMsg)
          .then((original) => {
            this.sendToIRCFromDiscord(message, original);
            this.sendToMatrixFromDiscord(message, original);
          })
          .catch(logger.error);
      } else {
    ...
    this.matrix.on('Room.timeline', (event) => {
      // ignore non messages, past messages, own messages
      if (event.getType() !== 'm.room.message' ||
        event.event.origin_server_ts <= this.matrixSyncTime ||
        event.sender.userId === this.matrix.credentials.userId) return;
      const author = event.sender.name;
      const roomId = event.event.room_id;
      const { body } = event.event.content;
      this.sendToDiscordFromMatrix(author, roomId, body);
      this.sendToIRCFromMatrix(author, roomId, body);
    });
  ...
  sendToIRCFromDiscord(message, inReplyTo) {
    ...
    if (ircChannel) {
      ...
      const text = this.parseText(message);
      let msgbody;
      if (inReplyTo && inReplyTo.author.id === this.discord.user.id) {
        // user is responding to a bridge reply from another service
        const extract = Bot.extractUserFromBridgedMessage(inReplyTo.content);
        msgbody = `in reply to **${extract.username}**\n`;
        msgbody += `> ${extract.message}\n${text}`;
      } else if (inReplyTo) {
        const replyNickname = Bot.getDiscordNicknameOnServer(inReplyTo.author, message.guild);
        msgbody = `in reply to **${replyNickname}@Discord**\n`;
        msgbody += `> ${inReplyTo.content}\n${text}`;
      } else {
        msgbody = text;
      }
      ...
      if (this.isCommandMessage(msgbody)) {
        patternMap.side = 'Discord';
        logger.debug('Sending command message to IRC', ircChannel, msgbody);
        ...
        this.ircClient.say(ircChannel, msgbody);
      } else {
        if (msgbody !== '') {
          // Convert formatting
          msgbody = formatFromDiscordToIRC(msgbody);
          patternMap.text = msgbody;

          msgbody = Bot.substitutePattern(this.formatIRCText, patternMap);
          logger.debug('Sending message to IRC', ircChannel, msgbody);
          this.ircClient.say(ircChannel, msgbody);
        }
  ...
  static extractUserFromBridgedMessage(message) {
    let match = message.match(/^\*{2}<(.+@.+)>\*{2} in reply to \*{2}.+@.+\*{2}\n> .+\n(.+)$/s);
    if (match) {
      // original message of bridged message is itself a reply
      return {
        username: match[1],
        message: match[2]
      };
  ...
  sendToMatrixFromDiscord(message, inReplyTo) {
    const { author } = message;
    // Ignore messages sent by the bot itself:
    if (author.id === this.discord.user.id ||
        Object.keys(this.webhooks).some(channel => this.webhooks[channel].id === author.id)
    ...
    if (inReplyTo && inReplyTo.author.id === this.discord.user.id) {
      // user is responding to a bridge reply from another service
      const extract = Bot.extractUserFromBridgedMessage(inReplyTo.content);
      msgbody = `<b>[${nickname}@Discord]</b> in reply to <b>${extract.username}</b><br>`;
      msgbody += `<blockquote>${extract.message}</blockquote>${message}`;
    } else if (inReplyTo) {
      const replyNickname = Bot.getDiscordNicknameOnServer(inReplyTo.author, fromGuild);
      msgbody = `<b>[${nickname}@Discord]</b> in reply to <b>${replyNickname}@Discord</b><br>`;
      msgbody += `<blockquote>${inReplyTo.content}</blockquote>${message}`;
    } else {
      msgbody = `<b>[${nickname}@Discord]</b> ${message}`;
    }
  ...
  sendToMatrixFromIRC(author, channel, message) {
    ...
    const msgbody = `<b>[${author}@IRC]</b> ${text}`;
    const content = {
      body: msgbody,
      format: 'org.matrix.custom.html',
      formatted_body: msgbody,
  ...
  static replaceMatrixReplies(message) {
    let replyMatch = message.match(MATRIX_BOT_REPLY_PATTERN);
    if (replyMatch !== null) {
    ...
    replyMatch = message.match(MATRIX_REPLY_PATTERN);
    if (replyMatch !== null) {
      const matrixUsername = replyMatch[1];
      const replyContent = replyMatch[2];
      const content = replyMatch[3];
      return `in reply to **${matrixUsername}@Matrix**\n> ${replyContent}\n\n${content}`;
    }
    return message;
  }
  ...
  sendToDiscordFromMatrix(author, roomId, message) {
    ...
    if (!discordChannel) return;
    const text = formatFromMatrixToDiscord(Bot.replaceMatrixReplies(message));
  ...
  sendToIRCFromMatrix(author, roomId, message) {
    ...
    if (this.ircNickColor) {
      const colorIndex = (author.charCodeAt(0) + author.length) % this.ircNickColors.length;
      displayUsername = irc.colors.wrap(this.ircNickColors[colorIndex], displayUsername);
    }
    const text = `<${displayUsername}@Matrix> ${formatFromMatrixToIRC(Bot.replaceMatrixReplies(message))}`;
